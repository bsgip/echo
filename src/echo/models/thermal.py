...
class HotWaterTank(Node):
    ...
    max_flow_rate: float  # Max hot water flow rate in litres per minute
    ins_transmittance: float = 0  # Thermal transmittance U-value of TES insulation in W/sqm*C
    surface_area: float = 0  # Surface area of TES in square meters, default value=0 means zero heat loss/gain
    ...
    def __init__(self, **data):
        ...
        self.ports["water_output"] = FlexSource(units=Units.LPS)
    ...
    @property
    def internal_temp(self):
        return "internal_temp" + self.node_name

    @property
    def conv_heat(self):
        return "convective_heat" + self.node_name

    @property
    def cond_heat(self):
        return "conductive_heat" + self.node_name

    @property
    def loss_gain(self):
        return "heat_loss_gain" + self.node_name
    ...
    def create_and_bound_temp_variable(self, model: EchoConcreteModel):
        ...
        for i in range(self.number_of_layers):
            setattr(model, f"{i}_{self.internal_temp}", en.Var(model.Expansion, model.Time, domain=en.NonNegativeReals))
        # Bound temp variable to be within range
    ...
    def apply_net_loss_and_gain_constraint(self, model: EchoConcreteModel):
        ...
        for i in range(self.number_of_layers):
            ...
            setattr(
                model,
                f"{i}_loss_gain_con1_{self.node_name}",
                en.Constraint(model.Expansion, model.Time, rule=net_loss_gain_constraint),
            )

    def apply_convective_heat_constraint(self, model: EchoConcreteModel):
        """Convective heat exchange in each layer happens due to inflow of water volume at different temperature"""
        water_port = self.ports["water_output"]
        volume_out = getattr(model, water_port.port_name)
        dt_sec = model.scenario_settings.interval_duration * 60
        ...
        for i in range(self.number_of_layers):
            ...
            setattr(
                model,
                f"{i}_dT_conv_{self.node_name}",
                en.Constraint(model.Expansion, model.Time, rule=convective_temp_change),
            )

    def apply_conductive_heat_constraint(self, model: EchoConcreteModel):
        ...
        if self.heat_to_top_only:
            heat_in_layer = {i: 0 for i in range(self.number_of_layers)}
            heat_in_layer[self.top_layer] = 1
        else:
            heat_in_layer = {i: 1 / self.number_of_layers for i in range(self.number_of_layers)}
        dt_sec = model.scenario_settings.interval_duration * 60
        # Calculate water tank radius and height in meters assuming Height/Diameter=3
        radius = np.cbrt(self.tank_volume / (np.pi * 6)) * 0.001
        height_layer = 6 * radius / self.number_of_layers
        # Thermal conductivity of water in W/m*C (value is at 25C, but assuming constant for simplicity)
        lambda_water = 0.6
        cond_scaling = lambda_water * np.pi * radius**2 / height_layer
        ...
        for i in range(self.number_of_layers):
            ...
            def conductive_heat_exchange(model: EchoConcreteModel, p, t):
                ...
                if i == self.top_layer:
                    ...
                    return getattr(model, f"{i}_dQ_cond_{self.node_name}")[p, t] == heat_in[p, t] * heat_in_layer[
                        i
                    ] + cond_scaling * (
                        getattr(model, f"{i-1}_{self.internal_temp}")[p, t - 1]
                        - getattr(model, f"{i}_{self.internal_temp}")[p, t - 1]
