...
class TimeVaryingPiecewiseIONode(InputOutputNode):
    ...
    def load_values_from_profile(self, model: EchoConcreteModel, profile_df: pd.DataFrame):
        ...
        if self.input_points_ref and self.input_points_ref in profile_df.columns:
            self.add_constant_input_points(
                profile_df[self.input_points_ref].to_list(), len(model.Time), len(model.Expansion)
            )
        ...
        if self.output_points_ref and self.output_points_ref in profile_df.columns:
            self.add_constant_output_points(
                profile_df[self.output_points_ref].to_list(), len(model.Time), len(model.Expansion)
            )
