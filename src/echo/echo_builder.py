"""
The echo builder module contains functions and classes used for building an echo model from a dict/json network
representation.
"""
...
class Network(EchoBaseModel):
    ...
    def add_node_to_components(
        ...
        if self.components.get(n_id) is not None:
            ...
            d = ComponentData(id=n_id, type=n_type, ports=ports, params=params, data=data)
            self.components[n_id] = d

    def add_edge_between_ports(
        self, node_tuple: tuple, port_tuple: tuple, resource: Optional[Units] = None, edge_name: Optional[str] = None
    ):
        """Adds an edge between two specified ports on two specified nodes."""
        e = EdgeData(nodes=node_tuple, ports=port_tuple, resource=resource)
    ...
    def add_objective(
        ...
        o = ObjectiveData(type=obj_type, name=name, component=component, prices=prices, charges=charges)
    ...
    def update_port_list_on_node(self, n_id: str, port: str):
        ...
        assert (
            type(self.components[n_id].ports) is list
    ...
    def update_port_dict_on_node(self, n_id: str, port_dict: dict):
        ...
        assert (
            type(self.components[n_id].ports) is dict
    ...
    def validate_new_edge(self, edge_name: str, node_tuple: tuple):
        ...
        for existing_edge_name, existing_edge_dict in self.edges.items():
            if existing_edge_dict.nodes == node_tuple:
                print(
                    'Nodes {} are already connected with existing edge named "{}". Ignoring current edge.'.format(
                        node_tuple, existing_edge_name
                    )
                )
                return None
            ...
            for _, node in self.components.items():
                if node1 == node.id or node2 == node.id:
                    if port1 not in node.ports and port2 not in node.ports:
                        err.append(
                            'In edge "{}", port "{}" doesn\'t belong to node "{}" or node "{}"'.format(
                ...
                for _, node in self.components.items():
                    if component_node == node.id:
                        ...
                        assert component_port in node.ports, (
                            ...
                            "exists on node {}, which has ports {}".format(
                                obj_name, component_node, component_port, component_node, node["ports"]
                            )
                        )
                if flag == 0:
                    raise ValueError(
                        'Objective "{}" refers to node "{}" but no node by that name is defined.'.format(
                            obj_name, component_node
                        )
                    )
        else:
...
class NetworkSet:
        ...
        for i in tqdm(range(self.num_networks), desc="Optimising sites", file=prog_file):
            ...
            self.results.append(
                process_single_network(
                    self.networks[i], self.interval_duration, self.time_periods, verbose=verbose, df=self.df
                )
            )
    ...
    def to_df(self, node, port) -> pd.DataFrame:
        """
        for each network segment in netset, gets the value of port on node, if that node and port combination does
        not exist
    ...
    def get_echo_model(self, network_name: str, df: Optional[pd.DataFrame] = None):
...
def process_single_network(
    network_dict: dict, interval_duration: int, time_periods: int, df: pd.DataFrame, verbose: bool = True
):
...
def convert_network_to_echo(netw: Network, df: Optional[pd.DataFrame] = None, verbose: bool = True):
...
def construct_echo_objective(system: OptimisationGraph, node_name_dict: dict, objective_dict: dict):
...
def run_echo_optimiser(
    ...
    for node_name, node_obj in echo_graph.node_obj.items():
        for port_name, port_obj in node_obj.ports.items():
            # Check we have the correct array lengths
            # todo may not be sufficient to just check initial value, what about other arrays
            array_length_check(
                port_obj.initial_value,
                time_periods,
                'port "{}" on node "{}", should have length = {} but has length = '.format(
                    port_name, node_name, time_periods
                ),
                scalar_ok=True,
            )
...
def create_battery_node(node_dict: dict):
    ...
    node = Battery(node_name=node_dict["id"], port_name=port_name, **node_dict["parameters"])
    return node
...
def create_tellegen_node(node_dict: dict, port_unit):
    ...
    if node_dict.get("parameters") is not None:
        for port_name, port_params in node_dict["parameters"].items():
            node.ports[port_name].set_flow_constraints(
                max_import=port_params.get("max_import"),
...
def create_flex_node(node_dict: dict, unit: Units) -> Node:
    ...
    port_name = check_node_has_only_one_port(node_dict)
    node = FlexNode(node_name=node_dict["id"], port_name=port_name, port_unit=unit)
    return node
...
def create_inverter_node(node_dict: dict) -> Node:
    ...
    assert set(ports_defined_on_node) == set(
        ports_defined_in_params
    ), 'Node "{}" has ports {} defined on node, and ports {} defined in parameters.'.format(
        node_dict["id"], ports_defined_on_node, ports_defined_in_params
    )
    inv_params = node_dict["parameters"]
    ac_port = inv_params.pop("ac_port_name")
    dc_ports = inv_params.pop("dc_port_names")
    inverter = NewInverter(node_name=node_dict["id"], ac_port_name=ac_port, dc_port_names=dc_ports)
    return inverter
...
def create_solar_node(node_dict: dict, df: pd.DataFrame) -> Node:
    ...
    if node_dict.get("parameters"):
        node = Solar(node_name=node_dict["id"], port_name=port_name, profile=pv_profile, **node_dict["parameters"])
    else:
...
def create_ev(node_dict: dict, df: pd.DataFrame) -> Node:
    ...
    node = EV(node_name=node_dict["id"], connection_port_name=cp_port_name, **ev_dict)  # pass all our params as kwargs
    return node
...
def get_tariff_component_from_node_port_name(tariff_dict: dict, node_name_dict: dict, system: OptimisationGraph):
    ...
    assert (
        target_node in node_name_dict.keys()
    ), 'Tariff component "{}" does not correspond to a defined node/port.'.format(tariff_dict["component"])
    node_obj = system.node_obj[node_name_dict[target_node]]
    component_obj = node_obj.ports[target_port]
    return component_obj


def add_flex_ports_to_node(node_obj: Node, port_list: list, port_units: list):
    for i in range(len(port_list)):
        new_port = FlexPort(units=port_units[i])
        node_obj.ports[port_list[i]] = new_port


def check_node_has_only_one_port(node_dict: dict) -> str:
    """Checks that a node has only one port defined"""
    ports = node_dict["ports"]
    if isinstance(ports, dict) or isinstance(ports, list):
        assert (
            len(ports) == 1
        ), 'Node {} is of type "{}" and can only have one port, but multiple are defined: {}.'.format(
...
def process_field(field, df):
    ...
    if type(field) is str:
        assert df is not None, "Dataframe must be provided, since field is defined by a dataframe column {}.".format(
            field
        )
        try:
            x = df[field]
...
def extract_results(optimiser: eo.EchoOptimiser, node_name_dict: dict) -> dict:
    ...
    for node_name, node_uid in node_name_dict.items():
        ...
        if "battery" in node_name:  # todo better way of doing this, we could refer to the node types?
            battery_node = cast(Battery, system.node_obj[node_uid])
            battery_port = cast(ElectricalStorage, battery_node.ports[list(battery_node.ports.keys())[0]])
            ...
            ev_node = cast(EV, system.node_obj[node_uid])
            vehicle = cast(MobileElectricalStorage, ev_node.ports["vehicle"])
            output[node_name]["vehicle_soc"] = optimiser.values(vehicle.soc_value, 0)
            output[node_name]["vehicle_p"] = optimiser.values(vehicle.port_name, 0)
            if hasattr(optimiser.model, vehicle.trip_slack):
                output[node_name]["trip_infeasibility"] = optimiser.values(vehicle.trip_slack, 0)
                output[node_name]["charge_status"] = (
                    "success" if all(output[node_name]["trip_infeasibility"] == 0) else "infeasible"
                )
            ...
            for port_name, port_obj in node_obj.ports.items():
                ...
                if hasattr(optimiser.model, port_obj.import_slack):
                    ...
                    output[node_name][port_name]["import_violation_max"] = optimiser.values(
                        port_obj.import_slack_max, 0
                    )
                else:
                ...
                if hasattr(optimiser.model, port_obj.export_slack):
                    ...
                    output[node_name][port_name]["export_violation_max"] = optimiser.values(
                        port_obj.export_slack_max, 0
                    )
                else:
                    pass
