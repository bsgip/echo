...
class SGT_interface:
    def __init__(self, network_file):
        ...
        con_point_df = pd.concat(
            [
                pd.DataFrame(
                    data=[
                        [zip.id(), zip.n_phases(), zip.bus().v_base(), zip.s_const()],
                    ],
                    columns=["id", "n_phases", "v_base", "s_const"],
                )
                for zip in netw.zips()
            ],
            ignore_index=True,
        )

        con_point_df["LV/MV"] = con_point_df["v_base"].apply(lambda x: "LV" if x <= 1.0 else "MV")
    ...
    def power_flows(self, power_factor=0.93, save_pickle_file=None, log_file=None, auto_taps=True):
        ...
        transformers = [
            sgt.Transformer.from_branch(br) for br in self.network.branches() if br.component_type() == "transformer"
        ]
        ...
        power_flow_results = {
            "status": status,
            "bus_voltage": bus_voltage_df,
            "branch_power_0": branch_power_0_df,
            "branch_power_1": branch_power_1_df,
            "branch_current_0": branch_current_0_df,
            "branch_current_1": branch_current_1_df,
            "transformer_names": transformer_names,
            "power_inbalance": p_inbalance_list,
            "total_generation": gen_total_list,
            "total_loss": loss_total_list,
            "total_zip": zip_total_list,
        }
    ...
    def load_series_from_df(self, df, mapping=None):
        ...
        assert (
            len(df.columns) == self.num_connections
        ), "dataframe must have one timeseries (column) per connection point"
        load_series = df.copy()
