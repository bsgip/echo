...
Most of this is the same as the scenario_example except that no network file is passed to teh netset creation and the optional parameter
energy_only is set to true

Each components has psuedo random assets (battery, evs, pv) at each components.
...
for i in range(num_sites):
    ...
    if has_battery[i]:
        components['battery'] = {'id': 'battery', 'type': NodeType.Battery, 'ports': ['battery'], 'parameters': battery_params.copy()}
    ...
    if has_solar[i]:
        ...
        components['solar'] = {'id': 'solar', 'type': NodeType.Solar, 'ports': ['solar'], 'data': pv, 'parameters': {'curtailable': False}}
    ...
    if has_ev[i]:
        ...
        for k in range(num_evs[i]):
            ...
            ev_name = tmp['name']
            components[ev_name] = {'id': ev_name, 'type': NodeType.EV, 'ports': ['ev_cp'], 'parameters': tmp}
