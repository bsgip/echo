...
electrical_grid = FlexNodeWithEmissions(
    node_name="ElectricalGrid",
    emitting_port="grid",
...
carbon_aggregation = CarbonAggregation(node_name="BulkEmissions", ports={"bulk_grid_emissions": CarbonPort()})

# Define all port for electrical connection point, omitting DC/AC Inverter model for now
electrical_cp_ports = {
    "CP_grid": FlexPort(units=Units.KW, export_constraint=FlowConstraint.Fixed, export_constraint_value=-175e3),
...
solar = Node(
    node_name="BulkSolar",
    ports={"solar": ElectricalGeneration(units=Units.KW, initial_value_ref="Solar", curtailable=False)},
)

wind = Node(
    node_name="BulkWind",
    ports={"wind": ElectricalGeneration(units=Units.KW, initial_value_ref="Wind", curtailable=False)},
)
...
electrolyser = Electrolyser(
    node_name="Electrolyser", input_port_unit=Units.KW, output_port_unit=Units.H2Kg, max_input=electrolyser_max_input
)
...
hydrogen_load = Node(
    node_name="H2Demand", ports={f"h2_demand": Demand(units=Units.H2Kg, initial_value_ref="Hydrogen load")}
)


hydrogen_cp_ports = {
    "CP_electrolyser": FlexPort(units=Units.H2Kg),
...
hydrogen_storage = Node(
    node_name="H2Storage",
    ports={
        "h2_storage_port": Storage(
            units=Units.H2Kg,
...
system.connect_ports_and_create_edge(
    wind.ports["wind"], connection_point.ports["CP_wind"], edge_name=f"{wind.node_name}_{connection_point.node_name}"
)
system.connect_ports_and_create_edge(
...
import_cost = ImportTariff(
    component=connection_point.ports["CP_grid"], tariff_array=source_df["Prices"] * 1e-3, expansion_periods=1
...
optimise_results = optimise(
    scenario_settings=ScenarioSettings(
        interval_duration=60, number_of_intervals=len(source_df.index), number_of_expansion_intervals=1, discount_rate=0
    ),
