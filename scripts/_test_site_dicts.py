...
test_load = np.array(
    [2.13, 2.09, 2.3, 2.11, 2.2, 2.23, 2.2, 2.15, 2.02, 2.19, 2.19, 2.19, 2.12, 2.15, 2.25, 2.12, 2.21, 2.16,
     ...
     2.19, 2.11, 2.17, 2.13, 2.05, 2.19] * 7)
...
available1 = np.array(([1] * 24 + [0] * 24 + [1] * 24 + [0] * 24) * 7)  # binary for when its available to charge
usage1 = np.array(([0.0] * 24 + [0.5] * 24 + [0.0] * 24 + [1.0] * 24) * 7)  # energy usage on trip at each time period
...
# Create vehicle 2
available2 = np.array(([1] * 10 + [0] * 10 + [1] * 28 + [0] * 48) * 7)
usage2 = np.array(([0.0] * 10 + [0.4] * 10 + [0.0] * 28 + [0.5] * 48) * 7)
...
import_tariff_array = np.array(([0.1] * 28 + [0.3] * 8 + [0.2] * 32 + [0.3] * 16 + [0.1] * 12) * 7)
export_tariff_array = np.array(([0.1] * 96) * 7)
...
scenario = {'name':'test_scenario', 'network':'ausnet_a',
            ...
            'sites':sites, 'interval_duration':15, 'time_periods':4*24*7}
