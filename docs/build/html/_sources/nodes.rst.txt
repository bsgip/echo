...
.. note::
    Under the heading for each node, **parameters**, **variables**, and **constraints** may be defined. **Parameters** are typically defined when the node is created, and can be controlled by the user. **Variables** are pyomo variables, and **constraints** are also pyomo constraints, and these are created when optimising a model.
...
Tellegen
^^^^^^^^^^^^^
A tellegen node is an energy conserving node that must have at least two ports. The sum of all flows through the node must equal 0. It is used to represent interconnections, either physical or logical, between different flows in a network.
...
The node constraint is the same as :ref:`Tellegen`, but a separate constraint is applied per commodity type.
...
The coefficients of performance are provided as parameters because they can be calculated prior to optimisation, based on ambient temperature data.
