...
class ImportTariff(Tariff):
    ...
    def objective_expr(self, model):
        return self.weight * sum(getattr(model, self.component.pos)[p, t] * getattr(model, self.import_tariff)[p, t] *
                                 model.interval_duration / 60 * getattr(model, model.dr)[p] for p in model.Expansion for
                                 t in
                                 model.Time)
...
class ExportTariff(Tariff):
    ...
    def objective_expr(self, model):
        return self.weight * sum(getattr(model, self.component.neg)[p, t] * getattr(model, self.export_tariff)[p, t] *
                                 model.interval_duration / 60 * getattr(model, model.dr)[p] for p in model.Expansion for
                                 t in
                                 model.Time)
