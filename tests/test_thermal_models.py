...
def test_thermal_storage_2_ports():
    ...
    cp = TellegenNode(node_name="conn_point",
                      ports={"to_supply_kwt": FlexPort(units=Units.KWT),
                             "to_demand_kwt": FlexPort(units=Units.KWT)})
    cp.ports.update({f"to_storage_{_port}_kwt": FlexPort(units=Units.KWT) for _port in storage_2p.ports.keys()})
    system = OptimisationGraph()
    system.add_node_obj([storage_2p, thermal_demand, thermal_mains, cp])
    system.connect_ports_and_create_edge(cp.ports["to_supply_kwt"], thermal_mains.ports["supply_kwt"])
    system.connect_ports_and_create_edge(cp.ports["to_storage_input_kwt"], storage_2p.ports["input"])
    system.connect_ports_and_create_edge(cp.ports["to_storage_output_kwt"], storage_2p.ports["output"])
    system.connect_ports_and_create_edge(cp.ports["to_demand_kwt"], thermal_demand.ports["demand_kwt"])
