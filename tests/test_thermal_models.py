...
def test_thermal_storage():
    ...
    optimise_results_no = optimise(
        ...
        graph=system, objective_set=objective_set)
...
def test_thermal_storage_2_ports():
    ...
    storage_2p = ThermalStorage(max_temp=80,
                                min_temp=10,
                                ambient_temp=ambient_temp_dict,
                                storage_mass=mass,
                                specific_heat=4184,
    ...
    cp_1 = TellegenNode(node_name="conn_point_supply",
                        ports={"to_supply_kwt": FlexPort(units=Units.KWT),
                               "to_demand_cp_kwt": FlexPort(units=Units.KWT),
                               f"to_storage_input_kwt": FlexPort(units=Units.KWT)})
    cp_2 = TellegenNode(node_name="conn_point_demand",
                        ports={"to_supply_cp_kwt": FlexPort(units=Units.KWT),
                               "to_demand_kwt": FlexPort(units=Units.KWT),
    ...
    objective_set = ObjectiveSet(objective_list=[ThroughputCost(component=storage_2p.ports[_port_name], rate=0.01)
    ...
    cp_flow_df_no = optimise_results_no.df_by_port()[[k
                                                for k in cp_1.ports.keys()]].reset_index(level=[0]).drop(
        columns='level_0')
    cp_flow_df_2 = optimise_results_no.df_by_port()[[k
                                                  for k in cp_2.ports.keys()]].reset_index(level=[0]).drop(
        columns='level_0')
    ...
    # Check that heat loss/gain to environment is calculated correctly
    assert all([round(loss_gain[k]) >= round((storage_2p.ambient_temp[k]
                                              -storage_temp[k])*storage_2p.lump_conductance)
                for k in loss_gain.keys()])

    # # Not expecting storage to do anything when no objective set, throughput cost shall prevent from random actions
    for storage_port in storage_2p.ports:
        assert (round(cp_flow_df_no[f"to_storage_{storage_port}_kwt"].min())
    ...
    cp_flow_df_pp = optimise_results_no.df_by_port()[[k
                                                for k in cp_1.ports.keys()]].reset_index(level=[0]).drop(
        columns='level_0')
    cp_flow_df_2 = optimise_results_no.df_by_port()[[k
                                                  for k in cp_2.ports.keys()]].reset_index(level=[0]).drop(
