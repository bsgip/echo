...
def test_sink_port_validation(initial_value, expected_error):
    initial_values_as_dict = {(0, i): v for i, v in enumerate(initial_value)}
    if expected_error:
        with pytest.raises(expected_error):
            Sink(initial_value=initial_values_as_dict)
    else:
        Sink(initial_value=initial_values_as_dict)
...
def test_sink_port_process_initial_value_performs_validation(initial_value, expected_error):
    if expected_error:
        with pytest.raises(expected_error):
            port = Sink()
            port.process_initial_value(initial_value, time_periods=len(initial_value))
